...
class KinematicsRobCoGen : public KinematicsBase<RBD::NJOINTS, typename ROBCOGEN::SCALAR>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

	KinematicsRobCoGen(std::shared_ptr<RBD> rbdContainer = std::shared_ptr<RBD>(new RBD()))
        : rbdContainer_(rbdContainer), KinematicsBase()
    {
        initEndeffectors(endEffectors_);
    }

    KinematicsRobCoGen(const KinematicsRobCoGen<RBD, N_EE>& other)
        : rbdContainer_(new RBD()), endEffectors_(other.endEffectors_)
    {
    }

    virtual ~KinematicsRobCoGen(){};

    KinematicsRobCoGen<RBD, N_EE>* clone() const { return new KinematicsRobCoGen<RBD, N_EE>(*this); }
     ...
     * @return
     */
    EndEffector<NJOINTS, SCALAR>& getEndEffector(size_t id) override { return endEffectors_[id]; };

    const HomogeneousTransforms& transformsRobCoGen() const { return robcogen().homogeneousTransforms(); }
    HomogeneousTransforms& transformsRobCoGen() { return robcogen().homogeneousTransforms(); }
    const Jacobians& jacobiansRobCoGen() const { return robcogen().jacobians(); }
    Jacobians& jacobiansRobCoGen() { return robcogen().jacobians(); }

    Velocity3Tpl getEEVelocityInBase(size_t eeId, const RBDState<NJOINTS, SCALAR>& rbdState) override
    {
    ...
    Position3Tpl getEEPositionInBase(size_t eeID,
        const typename JointState_t::Position& jointPosition) override
    {
    	return Position3Tpl(robcogen().getHomogeneousTransformBaseEEById(eeId, jointPosition).template topRightCorner<3, 1>());
    }
     ...
     */
    Matrix3Tpl getEERotInBase(size_t eeID, const typename JointState_t::Position& jointPosition) override
    {
        return robcogen().getHomogeneousTransformBaseEEById(eeId, jointPosition).template topLeftCorner<3, 3>();
    }
